...
def pending():
    ...
    views=DriftCollection(drifts,current_user.id)       #鱼漂页数据
...
def redraw_drift(did):
    with db.auto_commit():
        drift=Drift.query.filter_by(requester_id=current_user.id,id=did).first_or_404() #requester_id=current_user.id防止超权操作。12-21
...
def mailed_drift(did):
    with db.auto_commit():
        ...
        wish.launched=True                                  #用这两句就可以替代上面的    update()   语句
    return redirect(url_for('web.pending'))

def save_drift(drift_form, current_gift):   #插入Drift数据库
    with db.auto_commit():
        drift = Drift()
        # drift.message = drift_form.message.data
        drift_form.populate_obj(drift)      #一次性导入
